...
Use this guide for automated work in the `surgeist-text` crate repo.
...
This repo owns the `surgeist-text` crate.

The top-level `surgeist` repo is the facade crate and integration workspace. It links this repo as a submodule under `crates/surgeist-text` and owns workspace wiring, submodule pointers, cross-crate plans, source-derived API coordination, and whole-system verification.
...
`surgeist-text` owns text shaping, measurement, font-facing abstractions, and text layout contracts.
...
1. Confirm the work belongs in `surgeist-text`.
